...
class UploadDeepPlan(CotPlan):
    ...
    def system_prompt_addendum(self, ctx: PlanContext) -> str:
        ...
        size_note = (
            ...
            "  • documents_rag.list_project_documents — call FIRST when the "
            "user references uploaded files or asks to COMPARE/aggregate "
            "across files, so you know exactly which documents exist.\n"
            "  • documents_rag.read_document(document_id) — full text of ONE "
            "file; for a two-file comparison, list first then read EACH.\n"
            "  • documents_rag.search_project_documents — to find specific "
