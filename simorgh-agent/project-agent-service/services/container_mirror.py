...
async def mirror_message(project_id: str, role: str, content: str,
    ...
    Returns True if mirrored. Failures are logged but never raised — Postgres
    remains the source of truth and chat must keep working if the container
    is offline.
    """
    line = {
    ...
    # Use a small bash exec so we get atomic append. write_file would overwrite.
    script = (
