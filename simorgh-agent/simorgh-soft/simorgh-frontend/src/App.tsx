...
const Header: React.FC = () => {
  ...
  const handleSwitchRevision = async (revisionId: string) => {
    try {
      ...
      // Update project data in context with the snapshot from the selected revision
      if (projectSnapshot) {
        updateProjectData(projectSnapshot);
      }
...
const MainApp: React.FC<{ 
  ...
  const handleCreateRevision = async (revisionData: RevisionCreateData) => {
    ...
    if (result._id) {
      ...
      // Update project data in context with the snapshot from the new revision
      if (projectSnapshot) {
        updateProjectData(projectSnapshot);
      }
