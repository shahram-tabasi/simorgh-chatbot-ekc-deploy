...
CANONICAL_CARDS: List[Dict[str, Any]] = [
    ...
    {"doc_id": "leave", "topic": "leave_estehghaghi_overview",
     "title": "مرخصی استحقاقی — توضیح کامل و فهرست زیرنوع‌ها",
     "text": "مرخصی استحقاقی — توضیح کامل — استحقاقی چیست — مرخصی سالانه — annual leave overview.\n\n"
             "**تعریف:** مرخصی استحقاقی همان مرخصی سالانه با حقوق و مزایا است که هر کارگر/کارمند به‌صورت قانونی مستحق آن است (ماده ۶۴ قانون کار، بند ۶-۱ دستورالعمل مرخصی EKWI-AD-006-01).\n\n"
             "**۱) میزان (بند ۶-۱-۱)** — مطابق ماده ۶۴ قانون کار، هر ماه معادل **۲.۵ روز** مرخصی استحقاقی محسوب می‌شود که سالانه با احتساب ۴ جمعه جمعاً **۳۰ روز** می‌گردد. مرخصی به‌نسبت کارکرد محاسبه می‌شود؛ برای کارکرد کمتر از یک سال نیز به‌نسبت مدت کار محاسبه می‌گردد.\n"
             "  • **تبصره ۱:** روزهای تعطیل رسمی و استراحت پزشکی بین مرخصی، جزو مرخصی محسوب نمی‌شوند.\n"
             "  • **تبصره ۲:** ۱۱ اردیبهشت (روز کارگر) جزو مرخصی استحقاقی لحاظ شده و طبق قانون تمام کارگران تعطیل هستند.\n"
             "  • روز جمعه، روز تعطیل هفتگی با مزد است.\n"
             ...
             "**۲) ثبت مرخصی (بند ۶-۱-۲)** — کاربر باید از طریق سیستم کسرا به آدرس kasra.electrokavir.com نسبت به ثبت مرخصی خود اقدام نماید.\n\n"
             "**۳) ذخیره مرخصی (بند ۶-۱-۳)** — مطابق ماده ۶۹ قانون کار، تنها **۹ روز** از مرخصی سالانه قابل ذخیره است؛ مابقی استفاده‌نشده سوخت می‌شود. مرخصی ذخیره‌شده در پایان قرارداد قابل تبدیل به پول است و در صورت فوت به وارث پرداخت می‌گردد. تغییر این بند نیازمند مجوز مدیرعامل است (ماده ۶۶).\n\n"
             "**۴) استفاده از مرخصی (بند ۶-۱-۴)** — تاریخ و مدت استفاده منوط به درخواست همکار و موافقت مدیر مربوطه است. در صورت عدم موافقت مدیر و عدم حضور فرد در محل کار، آن روز **غیبت غیرموجه** تلقی می‌شود و مطابق آیین‌نامه انضباط کار با وی برخورد می‌گردد.\n\n"
             "**۵) زیرنوع‌های مرخصی استحقاقی** — مرخصی استحقاقی در دستورالعمل EKWI-AD-006-01 شامل **۷ زیرنوع** زیر است (برای جزئیات هر کدام می‌توانید همان نام را به‌تنهایی بپرسید):\n"
             "  • مرخصی روزانه (بند ۶-۱-۵-۱)\n"
             "  • مرخصی ساعتی (بند ۶-۱-۵-۲)\n"
             "  • مرخصی مجوز خروج (بند ۶-۱۰)\n"
             "  • مرخصی دوران شیردهی (بند ۶-۲، ماده ۷۸)\n"
             "  • مرخصی زایمان (بند ۶-۳، ماده ۷۶)\n"
             "  • مرخصی ازدواج و فوت (بند ۶-۴، ماده ۷۳)\n"
             "  • مرخصی تشویقی (بند ۶-۵)\n\n"
             "موارد جانبی مرتبط: **بازخرید مرخصی** (بند ۶-۱۱) و **مرخصی منفی** حداکثر ۳ روز.",
     "source_section": "۶-۱- مرخصی استحقاقی (بندهای ۶-۱-۱ تا ۶-۱-۵)"},
    {"doc_id": "leave", "topic": "anchor_estehghaghi",
     "title": "مرخصی استحقاقی — خلاصه",
     "text": "مرخصی استحقاقی چیست؟ توضیح مرخصی استحقاقی؟ شرایط مرخصی سالانه؟\n\n"
             "مرخصی استحقاقی = **۳۰ روز سالانه** با حقوق و مزایا (ماده ۶۴ قانون کار، بند ۶-۱ دستورالعمل EKWI-AD-006-01). ماهانه ۲.۵ روز. حداکثر ۹ روز قابل ذخیره (ماده ۶۹).\n"
             "**شامل ۷ زیرنوع:** روزانه، ساعتی، مجوز خروج، شیردهی، زایمان، ازدواج و فوت، تشویقی.\n"
             "ثبت در سامانه کسرا (kasra.electrokavir.com). برای جزئیات هر زیرنوع همان نام را بپرسید.",
     "source_section": "خلاصه مرخصی استحقاقی"},
    ...
    {"doc_id": "leave", "topic": "leave_hourly",
     "title": "مرخصی ساعتی — توضیح کامل و سقف‌ها",
     "text": "مرخصی ساعتی — توضیح کامل — hourly leave full.\n\n"
             "**تعریف (بند ۶-۱-۵-۲-۱):** کارمند می‌تواند **یک ساعت یا بیشتر** را در طول روز در محل کار حضور نداشته باشد و ساعاتی که مرخصی گرفته‌است از مقدار زمان مجاز تعطیلی سالیانه فرد کسر می‌شود. این تعطیلی برای افرادی است که کار بسیار واجب و ضروری در خارج از شرکت داشته یا مشکلی برایشان پیش آمده و مجبور به رسیدگی هستند.\n\n"
             "**۱) مبنای قانونی (بند ۶-۱-۵-۲-۲)** — طبق **ماده ۷ قانون کار**، مرخصی کمتر از یک روز کار، جزو **مرخصی استحقاقی** منظور می‌شود. به میزان ساعتی که از مرخصی ساعتی استفاده شود، از مرخصی‌های استحقاقی کاهش پیدا می‌کند.\n\n"
             "**۲) سقف روزانه (بند ۶-۱-۵-۲-۳)** — استفاده از مرخصی ساعتی برای هر کارمند در طول یک روز کاری **حداکثر تا سقف ۴ ساعت** مجاز است. در صورتی که مدت غیبت ناشی از مرخصی ساعتی در یک روز از ۴ ساعت فراتر رود، کارمند موظف است:\n"
             "  • آن روز را به‌عنوان **مرخصی استحقاقی روزانه** ثبت نماید\n"
             "  • مدت زمانی که در شرکت حضور داشته به‌عنوان **اضافه‌کار** در نظر گرفته می‌شود\n\n"
             "**۳) سقف ماهانه (بند ۶-۱-۵-۲-۴)** — سقف استفاده از مرخصی ساعتی برای هر کارمند، **حداکثر ۵ نوبت در ماه** می‌باشد.\n\n"
             "**۴) استفاده بیش از سقف (تبصره ۵)** — در صورت نیاز به استفاده بیش از سقف مذکور، مراتب صرفاً با درخواست کارمند و با تأیید سه‌گانه قابل موافقت است:\n"
             "  • تأیید **مدیر مستقیم**\n"
             "  • تأیید **معاونت مربوطه**\n"
             "  • تأیید **مدیرعامل** یا نماینده ایشان\n\n"
             "**۵) ۱۰ دقیقه اول (بند ۶-۱-۵-۲-۵)** — در صورت تأخیر در حضور در محل کار، امکان ثبت مجوز مرخصی برای **۱۰ دقیقه نخست** آغاز کار وجود نداشته و این مدت به‌عنوان **کسرکار** منظور خواهد شد.\n\n"
             "**موارد مرتبط:** مرخصی ساعتی یکی از زیرنوع‌های **مرخصی استحقاقی** است. ثبت در سامانه کسرا (kasra.electrokavir.com).",
     "source_section": "۶-۱-۵-۲- مرخصی ساعتی (بندهای ۶-۱-۵-۲-۱ تا ۶-۱-۵-۲-۵)"},
    {"doc_id": "leave", "topic": "leave_maternity",
     "title": "مرخصی زایمان — توضیح کامل (مدت، مزد، شرایط)",
     "text": "مرخصی زایمان — توضیح کامل — maternity leave full.\n\n"
             "**تعریف:** مطابق قانون برای مادران شاغل، به مدت **۹ ماه** مرخصی زایمان تعلق می‌گیرد. مرخصی زایمان جزء **مرخصی‌های با حقوق** به حساب می‌آید و روزهای آن در ردیف روزهای بیمه برای بازنشستگی محاسبه می‌شود (بند ۶-۳ دستورالعمل مرخصی EKWI-AD-006-01).\n\n"
             "**۱) مدت زمان (بند ۶-۳-۱)** — مدت زمان مرخصی زایمان مطابق قوانین جاری کشور است. طبق **ماده ۷۶ قانون کار**، مرخصی زایمان کارگر زن جمعاً **۹ ماه** است و حتی‌الامکان **۴۵ روز** از این مرخصی باید پس از زایمان مورد استفاده قرار گیرد.\n\n"
             "**۲) مزد ایام مرخصی زایمان (بند ۶-۳-۲)** — مبلغ کمک‌هزینه مرخصی زایمان معادل میانگین (**دو سوم**) دستمزد روزانه بیمه‌شده در آخرین **۹۰ روز** قبل از شروع استراحت ایام بیماری برای هر روز استراحت است. حداکثر مدت کمک مزبور **۹ ماه** است (حداکثر زمان استفاده از این مرخصی **قبل از زایمان ۲ ماه** می‌باشد) **بدون کسر ۳ روز اول**. در طول این مدت، پرداخت حقوق و مزایای کارگر بر عهده **سازمان تأمین اجتماعی** است (طبق مقررات قانون تأمین اجتماعی).\n\n"
             "**مزایای جانبی که در ایام مرخصی زایمان همچنان به همکار تعلق می‌گیرد:**\n"
             "  • کمک هزینه **عائله‌مندی**\n"
             "  • **عیدی**\n"
             "  • مزایای **پایان کار**\n"
             "  • **مرخصی استحقاقی**\n\n"
             "**۳) شرایط استفاده از مرخصی زایمان (بند ۶-۳-۳)** — برای استفاده از این مرخصی، چهار شرط زیر باید برقرار باشد:\n"
             "  ۱. ظرف **یکسال پیش از زایمان**، حق بیمه **۶۰ روز کار** را به سازمان تأمین اجتماعی پرداخت کرده باشند.\n"
             ...
             "**موارد مرتبط:** پس از پایان مرخصی زایمان، مادران شاغل تا پایان دو سالگی کودک از **مرخصی دوران شیردهی** برخوردارند (بند ۶-۲، ماده ۷۸). مرخصی زایمان یکی از زیرنوع‌های **مرخصی استحقاقی** است (شامل ۷ زیرنوع: روزانه، ساعتی، مجوز خروج، شیردهی، زایمان، ازدواج و فوت، تشویقی).",
     "source_section": "۶-۳- مرخصی زایمان (بندهای ۶-۳-۱، ۶-۳-۲، ۶-۳-۳)"},
    {"doc_id": "leave", "topic": "leave_lactation",
     "title": "مرخصی دوران شیردهی — توضیح کامل",
     "text": "مرخصی دوران شیردهی — حق شیر — توضیح کامل — lactation leave full.\n\n"
             "**مبنای قانونی:** طبق **ماده ۷۸ قانون کار**، کارفرما مکلف است به مادران شیرده تا پایان دو سالگی کودک، **پس از هر سه ساعت، نیم ساعت** فرصت شیردادن بدهد. این فرصت **جزء ساعات کار** آنان محسوب گردیده و **از مرخصی استحقاقی آنان کسر نخواهد شد** (بند ۶-۲ دستورالعمل مرخصی EKWI-AD-006-01).\n\n"
             "**مقدار حق شیر در شرکت الکتروکویر:**\n"
             "  • خانم‌های مشغول در **کارخانه**: روزانه **۰۱:۱۵** (یک ساعت و پانزده دقیقه)\n"
             "  • خانم‌های مشغول در **دفتر مرکزی تهران**: روزانه **۰۱:۳۰** (یک ساعت و نیم)\n"
             "  • تا **دو سالگی** فرزند\n\n"
             "**فرزندان دو قلو یا چند قلو:** مادرانی که صاحب فرزندان دو قلو یا چند قلو هستند، روزانه به مدت **۲ ساعت** از مرخصی ساعتی می‌توانند برخوردار شوند.\n\n"
             "**نحوه استفاده:** به‌صورت **شناور**، می‌توان طی ساعات موظفی بر اساس **توافق فی‌مابین مدیر واحد و پرسنل** استفاده نمود.\n\n"
             "**محدودیت‌های مهم:** درخواست مادران مبنی بر موارد زیر **امکان ندارد**:\n"
             "  • عدم استفاده از این حق و اضافه نمودن آن به مرخصی استحقاقی سالانه\n"
             "  • تبدیل آن به وجه نقد\n"
             "  • منظور داشتن آن به‌عنوان اضافه‌کاری\n\n"
             "**موارد مرتبط:** مرخصی شیردهی یکی از زیرنوع‌های **مرخصی استحقاقی** است و معمولاً پس از پایان **مرخصی زایمان** (بند ۶-۳، ۹ ماه) آغاز می‌شود.",
     "source_section": "۶-۲- مرخصی دوران شیردهی"},
    {"doc_id": "leave", "topic": "leave_marriage_bereavement",
     "title": "مرخصی ازدواج و فوت — توضیح کامل",
     "text": "مرخصی ازدواج و فوت — توضیح کامل — marriage and bereavement leave full.\n\n"
             "**مبنای قانونی:** طبق **ماده ۷۳ قانون کار**، به کلیه کارگران در موارد زیر حق داده شده که از **۳ روز مرخصی** استفاده نمایند (بند ۶-۴ دستورالعمل مرخصی EKWI-AD-006-01):\n\n"
             "**الف) ازدواج دائم** — ۳ روز مرخصی.\n\n"
             "**ب) فوت همسر، پدر، مادر یا فرزندان** — ۳ روز مرخصی برای فوت اقوام درجه یک.\n\n"
             "**نکته مهم:** مرخصی ازدواج و فوت اقوام درجه یک **به هیچ عنوان جزو مرخصی استحقاقی** به حساب نمی‌آید — یعنی این ۳ روز خارج از سقف ۳۰ روز سالانه مرخصی استحقاقی است و از مانده مرخصی کسر نمی‌شود.\n\n"
             "**موارد مرتبط:** مرخصی ازدواج و فوت یکی از زیرنوع‌های **مرخصی استحقاقی** در دستورالعمل است (هرچند از سقف استحقاقی کسر نمی‌شود). ثبت در سامانه کسرا (kasra.electrokavir.com).",
     "source_section": "۶-۴- مرخصی استحقاقی ازدواج و فوت"},
    {"doc_id": "leave", "topic": "leave_sick",
     "title": "مرخصی استعلاجی — توضیح کامل",
     "text": "مرخصی استعلاجی — توضیح کامل — sick leave full.\n\n"
             "**تعریف:** هرگونه بیماری کارگر اعم از بیماری عادی، ناشی از کار یا حوادث کار، در صورت تجویز **پزشک معالج** مستوجب استفاده کارگر از مرخصی استعلاجی می‌شود (بند ۶-۶ دستورالعمل مرخصی EKWI-AD-006-01).\n\n"
             "**۱) مدت** — در قانون مدت خاصی برای این نوع مرخصی ذکر نگردیده است؛ به هر میزان که پزشک معالج تجویز کرده باشد تا زمان بازیابی **سلامتی کامل** کارگر با تأیید سازمان تأمین اجتماعی قابل استفاده است.\n\n"
             "**۲) غرامت دستمزد و شرایط ماده ۵۹ تأمین اجتماعی** — به کارگرانی که از این نوع مرخصی استفاده می‌کنند، به شرط تحقق شرایط مندرج در **ماده ۵۹ قانون تأمین اجتماعی**، غرامت دستمزد تعلق می‌گیرد. پرداخت غرامت توسط **سازمان تأمین اجتماعی** انجام می‌شود (نه شرکت). شرایط ماده ۵۹:\n"
             "  • **الف)** بیمه‌شده بر اثر حوادث ناشی از کار یا بیماری‌های حرفه‌ای تحت درمان قرار گرفته باشد و از کار افتاده کلی نباشد.\n"
             "  • **ب)** در صورتی که بیمه‌شده بر سبب بیماری و طبق گواهی پزشک، احتیاج به استراحت مطلق یا بستری شدن داشته باشد و در تاریخ اعلام بیماری، مشغول به کار بوده یا در مرخصی استحقاقی باشد.\n"
             ...
             "**۳) شروع پرداخت غرامت** — غرامت دستمزد از **اولین روز** که بیمه‌شده بر اثر حادثه یا بیماری حرفه‌ای قادر به کار نباشد، پرداخت می‌شود. در مواردی که عدم اشتغال به کار به سبب بیماری باشد و بیمار در بیمارستان بستری نشود، غرامت دستمزد از **روز چهارم** پرداخت خواهد شد.\n\n"
             "**۴) ثبت** — این نوع مرخصی **حتماً باید در سامانه خدمات غیرحضوری سازمان تأمین اجتماعی ثبت گردد** تا در صورت نیاز به تأیید پزشک معتمد این سازمان، مرخصی تجویزشده از سوی پزشک معالج به تأیید برسد تا رسمیت یابد.\n\n"
             "**۵) محسوب شدن در سوابق (ماده ۷۴)** — طبق **ماده ۷۴ قانون کار**، مدت مرخصی استعلاجی با تأیید سازمان تأمین اجتماعی، **جزو سوابق کار و بازنشستگی** کارگران محسوب می‌شود. بنابراین به این ایام **کمک عائله‌مندی، عیدی و حق سنوات** نیز تعلق می‌گیرد.\n\n"
             "**۶) تبدیل استحقاقی به استعلاجی (بند ۶-۶-۱)** — کارکنانی که در حال استفاده از مرخصی استحقاقی هستند، در صورت ابتلا به بیماری و مصداق مواد مربوطه، حکم مرخصی استحقاقی آنان از تاریخ ابتلا به مرخصی استعلاجی تبدیل می‌گردد.\n\n"
             "**۷) حقوق در ایام مرخصی استعلاجی (بند ۶-۶-۲)** — جهت ایام مرخصی استعلاجی **حقوق از طرف شرکت پرداخت نمی‌گردد** (غرامت فقط از تأمین اجتماعی).",
     "source_section": "۶-۶- مرخصی استعلاجی (بندهای ۶-۶-۱، ۶-۶-۲ + ماده ۵۹ و ۷۴)"},
    ...
    {"doc_id": "leave", "topic": "leave_unpaid_overview",
     "title": "مرخصی بدون حقوق — توضیح کامل و زیرنوع",
     "text": "مرخصی بدون حقوق — توضیح کامل — بدون حقوق چیست — unpaid leave overview.\n\n"
             "**تعریف:** حق استفاده کارکنان از مرخصی در روزهای کاری بدون استفاده از حقوق و مزایا (بند ۵-۳ و ۶-۷ دستورالعمل EKWI-AD-006-01). به موجب ماده ۱۶ قانون کار، قرارداد کارگر در طول مرخصی بدون حقوق به حالت تعلیق در می‌آید و پس از پایان دوره، می‌تواند مجدداً مشغول به کار شود.\n\n"
             "**۱) شرایط و میزان (بند ۶-۷-۱)** — مطابق ماده ۷۲ قانون کار، کارکنان در موارد اضطراری و با موافقت شرکت می‌توانند حداکثر **یک‌دوازدهم سنوات خدمت** خود (متوالی یا منقطع) از مرخصی بدون حقوق استفاده نمایند:\n"
             "  • حداقل **یک سال سابقه خدمت** الزامی است.\n"
             "  • با اخذ مجوز مدیرعامل، امکان افزایش مدت بیش از مقدار مصوب وجود دارد.\n\n"
             "**۲) تأثیر بر سابقه (بند ۶-۷-۲)** — جزو سابقه خدمت محسوب نمی‌شود و هیچ‌گونه مزایایی در مدت مرخصی به مستخدم تعلق نمی‌گیرد.\n\n"
             "**۳) ضرورت استفاده (بند ۶-۷-۳)** — هرگاه مرخصی استحقاقی به‌طور کامل استفاده شده باشد، با توافق کارفرما، کارمند می‌تواند از مرخصی بدون حقوق استفاده کند. این مرخصی تنها با موافقت کارفرما یا نماینده وی امکان‌پذیر است و نیازمند درخواست کتبی و تأیید مدیریت است.\n\n"
             "**۴) سقف (بند ۶-۷-۴)** — سقف مجاز در یک سال کاری: **یک ماه**.\n"
             "  • تا **۷ روز**: نیاز به تأیید سرپرست/مدیر و تصویب در کمیته منابع انسانی.\n"
             "  • بیش از **۷ روز**: نیاز به موافقت مدیر مربوطه و تأیید مدیرعامل یا نماینده وی.\n\n"
             "**۵) زیرنوع: مرخصی تحصیلی (بند ۶-۹)** — مرخصی تحصیلی نوعی مرخصی بدون حقوق است که به افرادی تعلق می‌گیرد که از طرف شرکت ملزم به ادامه تحصیل می‌شوند. مدت قانونی: فوق‌دیپلم ۲ سال، کارشناسی ۴ سال، کارشناسی‌ارشد ۲ سال؛ قابل تمدید ۲ سال دیگر در هر مقطع. در الکتروکویر **تهاتر با اضافه‌کاری** الزامی است؛ در صورت نداشتن اضافه‌کار یا مرخصی برای تهاتر، کسرکار محسوب می‌شود. اخذ مجوز مدیرعامل یا نماینده وی الزامی است.\n\n"
             "موارد مرتبط: **مرخصی حج عمره** نیز با دلیل سفر طولانی به‌صورت مرخصی بدون حقوق قابل استفاده است (بند ۶-۸).",
     "source_section": "۶-۷- مرخصی بدون حقوق + ۶-۹- مرخصی تحصیلی"},
    ...
    {"doc_id": "leave", "topic": "leave_all_types",
     "title": "فهرست کامل و ساختار رسمی انواع مرخصی در شرکت الکتروکویر",
     "text": "انواع مرخصی — انواع مرخصی شرکت الکتروکویر — لیست همه مرخصی‌ها — فهرست مرخصی‌های مجاز — مرخصی چه نوع‌هایی دارد — kinds of leave — leave types. "
             "ساختار رسمی انواع مرخصی در شرکت الکتروکویر طبق دستورالعمل مرخصی (کد EKWI-AD-006-01) شامل **چهار دسته اصلی** است:\n\n"
             "**۱) مرخصی استحقاقی** — ۳۰ روز سالانه با احتساب ۴ جمعه (ماده ۶۴ قانون کار)، ماهانه ۲.۵ روز، حداکثر ۹ روز قابل ذخیره (ماده ۶۶ و ۶۹). شامل زیرنوع‌های زیر است:\n"
             "  • مرخصی روزانه (کارخانه ۰۷:۲۰ / تهران ۰۸:۳۰ معادل یک روز — بند ۶-۱-۵-۱)\n"
             "  • مرخصی ساعتی (حداکثر ۴ ساعت در روز، حداکثر ۵ نوبت در ماه — بند ۶-۱-۵-۲)\n"
             "  • مرخصی مجوز خروج (حداکثر ۱۰ دقیقه، دو بار در هفته — بند ۶-۱۰)\n"
             "  • مرخصی دوران شیردهی (کارخانه ۰۱:۱۵ روزانه، تهران ۰۱:۳۰ روزانه تا دو سالگی کودک، دوقلو ۲ ساعت — بند ۶-۲، ماده ۷۸)\n"
             "  • مرخصی زایمان (جمعاً ۹ ماه با حقوق از تأمین اجتماعی — بند ۶-۳، ماده ۷۶)\n"
             "  • مرخصی ازدواج و فوت (۳ روز ازدواج دائم، ۳ روز فوت اقوام درجه یک — بند ۶-۴، ماده ۷۳)\n"
             "  • مرخصی تشویقی (با درخواست مدیر واحد و موافقت مدیرعامل برای کارکنان کوشا یا دارای نوآوری — بند ۶-۵)\n\n"
             "**۲) مرخصی استعلاجی** — با تأیید پزشک معالج و سازمان تأمین اجتماعی، مدت قانونی مشخص ندارد، حقوق توسط شرکت پرداخت نمی‌شود، غرامت دستمزد از تأمین اجتماعی پرداخت می‌شود (در بستری از روز اول، در سایر موارد از روز چهارم)، ثبت در سامانه خدمات غیرحضوری تأمین اجتماعی الزامی است (بند ۶-۶، ماده ۵۹ تأمین اجتماعی).\n\n"
             "**۳) مرخصی بدون حقوق** — سقف سالانه ۱ ماه، تا یک‌دوازدهم سنوات خدمت (ماده ۷۲)؛ تا ۷ روز با تأیید سرپرست و کمیته منابع انسانی، بیش از ۷ روز با مدیرعامل. شرط: حداقل یک سال سابقه. جزو سابقه خدمت محسوب نمی‌شود (بند ۶-۷). شامل زیرنوع:\n"
             "  • مرخصی تحصیلی (نوع بدون حقوق، فوق‌دیپلم ۲ سال، کارشناسی ۴ سال، کارشناسی‌ارشد ۲ سال، قابل تمدید ۲ سال در هر مقطع، با مجوز مدیرعامل، در الکتروکویر تهاتر با اضافه‌کاری الزامی — بند ۶-۹)\n\n"
             "**۴) مرخصی حج** — یک ماه مرخصی با حقوق فقط برای حج تمتع واجب (ماده ۶۷)؛ برای حج عمره مرخصی بدون حقوق با دلیل سفر طولانی (بند ۶-۸).\n\n"
             "موارد جانبی مرتبط: **بازخرید مرخصی** (بند ۶-۱۱، در صورت عدم موافقت مدیر یا خاتمه همکاری) و **سایر مرخصی‌ها** (مسابقات ورزشی با مجوز مدیران ارشد — بند ۶-۱۲). "
             "ثبت همه مرخصی‌ها از طریق سامانه کسرا (kasra.electrokavir.com) با تأیید مدیر مستقیم. مرجع کامل: دستورالعمل مرخصی (EKWI-AD-006-01)، فصل ۵ و فصل ۶.",
     "source_section": "۵. انواع مرخصی + ۶-۱ تا ۶-۱۲"},
    ...
    {"doc_id": "leave", "topic": "leave_incentive",
     "title": "مرخصی تشویقی و آموزشی — توضیح کامل",
     "text": "مرخصی تشویقی و آموزشی — توضیح کامل — incentive leave full.\n\n"
             "**تعریف (بند ۶-۵):** در صورت **درخواست مدیر واحد** و با **موافقت مدیرعامل**، به منظور تشویق کارکنان کوشا و کارکنانی که با نوآوری خود موجب افزایش بهره‌وری می‌شوند یا پیشنهاد سازنده و قابل اجرایی داشته باشند، **مرخصی تشویقی** قابل اعطاء است. برای ارتقاء سطح دانش آنان، **مرخصی آموزشی** نیز قابل اعطاء می‌باشد.\n\n"
             "**شرایط اعطا:**\n"
             "  • درخواست **مدیر واحد** (الزامی)\n"
             "  • موافقت **مدیرعامل** (الزامی)\n"
             "  • دلیل: تشویق کارکنان کوشا، یا نوآوری منجر به افزایش بهره‌وری، یا پیشنهاد سازنده و قابل اجرایی\n\n"
             "**کار در ایام تعطیل:** در صورت انجام مأموریت یا کار در هر روز از ایام تعطیلات رسمی، **حقوق طبق قانون پرداخت** خواهد شد.\n\n"
             "**تبصره ۶:** تعطیلات رسمی همان است که در **تقویم رسمی کشور** مشخص شده یا به موجب **مصوبه هیأت دولت** اعلام می‌شود.\n\n"
             "**تبصره ۷:** **تعطیلات توافقی** شامل تعطیلات عید نوروز و تعطیلات تابستانی با **تأیید مدیرعامل** می‌باشد.\n\n"
             "**موارد مرتبط:** مرخصی تشویقی یکی از زیرنوع‌های **مرخصی استحقاقی** است.",
     "source_section": "۶-۵- مرخصی تشویقی + تبصره‌های ۶ و ۷"},
    {"doc_id": "leave", "topic": "leave_study",
     "title": "مرخصی تحصیلی — توضیح کامل",
     "text": "مرخصی تحصیلی — توضیح کامل — study leave full.\n\n"
             "**تعریف (بند ۶-۹):** مرخصی تحصیلی از نوع **مرخصی‌های بدون حقوق** می‌باشد. ویژگی حائز اهمیت این نوع مرخصی برخلاف دیگر مرخصی‌های بدون حقوق این است که به آن دسته از افرادی تعلق می‌گیرد که **از طرف شرکت** جهت رشد و ارتقا **ملزم به ادامه تحصیل** می‌گردند.\n\n"
             "**مدت زمان قانونی برای هر مقطع:**\n"
             "  • **فوق‌دیپلم**: ۲ سال\n"
             "  • **کارشناسی**: ۴ سال\n"
             "  • **کارشناسی ارشد**: ۲ سال\n"
             "  • قابل تمدید **۲ سال دیگر** در همان مقطع\n\n"
             "**شرط خاص شرکت الکتروکویر:** **تهاتر مرخصی تحصیلی با اضافه‌کاری** الزامی است.\n\n"
             "**تبصره ۸:** جهت تحصیل لازم است **مجوز مدیرعامل** یا نماینده وی اخذ گردد.\n\n"
             "**تبصره ۹:** کارگر جهت استفاده از مرخصی تحصیلی لازم است **به همان مقدار اضافه‌کار** داشته باشد، در غیر این صورت از مرخصی کسر می‌گردد.\n\n"
             "**تبصره ۱۰:** در صورت **نداشتن اضافه‌کار و مرخصی** جهت تهاتر با مرخصی تحصیلی، برای فرد **کسرکار** منظور خواهد شد.\n\n"
             "**موارد مرتبط:** مرخصی تحصیلی زیرنوع **مرخصی بدون حقوق** است (بند ۶-۷). حقوق و مزایا در این دوره پرداخت نمی‌شود و جزو سابقه خدمت محسوب نمی‌گردد.",
     "source_section": "۶-۹- مرخصی تحصیلی + تبصره‌های ۸، ۹، ۱۰"},
    ...
    {"doc_id": "leave", "topic": "anchor_leave_types",
     "title": "انواع مرخصی — ساختار کامل",
     "text": "انواع مرخصی در شرکت الکتروکویر؟ چه نوع مرخصی‌هایی موجود است؟ فهرست مرخصی‌ها چیست؟ ساختار رسمی مرخصی چیست؟\n\n"
             "طبق دستورالعمل EKWI-AD-006-01، شرکت الکتروکویر **۴ دسته اصلی مرخصی** دارد:\n"
             "**۱) مرخصی استحقاقی** — شامل: مرخصی روزانه، مرخصی ساعتی، مرخصی مجوز خروج، مرخصی دوران شیردهی، مرخصی زایمان، مرخصی ازدواج و فوت، مرخصی تشویقی.\n"
             "**۲) مرخصی استعلاجی**\n"
             "**۳) مرخصی بدون حقوق** — شامل: مرخصی تحصیلی.\n"
             "**۴) مرخصی حج**\n\n"
             "موارد جانبی: بازخرید مرخصی و سایر مرخصی‌ها (ورزشی) با مجوز مدیران ارشد. جزئیات هر نوع در کارت اختصاصی همان نوع است.",
     "source_section": "ساختار رسمی انواع مرخصی"},
