...
Please answer the current question, keeping in mind the context from our previous conversation."""
    ...
    # outbound call. Tunable: set OPENAI_HEALTH_TTL=0 to force re-checks.
    ...
    def _check_openai_health(self) -> Dict[str, Any]:
        ...
        Non-blocking by design: result is cached for OPENAI_HEALTH_TTL
        seconds, and the underlying API call uses a short
        OPENAI_HEALTH_TIMEOUT so a slow VPN can't stall /health.
        """
        import time
        ...
        # Treat the well-known placeholder as unconfigured to avoid
        # endless retries against a key that will never work.
        ...
        try:
            # Tight timeout — health probe, not a real call.
            ...
            # Don't spam ERROR — this runs on every /health hit.
